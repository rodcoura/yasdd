...
yasdd 是纯 markdown —— 无构建步骤, 无依赖。把 `skills/` 和 `agents/` 放到你的 agent harness 读取的位置。

### 推荐: 安装脚本

`install-to-agents.sh` 脚本将 skills 和 agents 复制到每个工具实际扫描的位置:
...
| 目录 | 内容 |
| --- | --- |
| `~/.agents/` | skills, agents (跨工具镜像) |
| `~/.config/opencode/` | agents (opencode 原生) |
| `~/.claude/` | agents, skills (Claude Code 原生) |

### 手动: 软链接

任选其一:

- **全局(所有项目):** `~/.agents/` —— 即 `~/.agents/skills/yasdd-*/SKILL.md` 与 `~/.agents/agents/yasdd-spy.md`。
- **项目内(单个仓库):** 项目根目录的 `.agents/` —— `.agents/skills/...` 与 `.agents/agents/...`。
- **自定义:** 你的 agent harness 加载 skills 的任何文件夹。

为每个 skill + agent 建立软链接(若该文件夹已存有其他 skills, 此操作安全):
...
项目内安装则把循环中的 `~/.agents/` 换成 `.agents/`。然后在项目中加载 `yasdd-init` skill 创建 `.yasdd/` 并更新 `AGENTS.md`。

---

## yasdd 是什么?

yasdd 是一个**规范驱动开发框架**, 完全由 markdown 技能构成。它为 AI 编程代理提供了一条可重复的流水线, 把模糊的功能需求转化为完全实现并经过评审的功能——既不过度思考, 也不跳过棘手的问题。

它不是源代码, 也没有构建系统。一切都在两个目录中:

- `skills/` — 每个技能一个文件夹 (`<skill>/SKILL.md`); 包含编排器 playbook 和子代理指令。
- `agents/` — 子代理定义(目前为 `yasdd-spy.md`, 轻量级代码库探索器)。
...
| 技能 | 作用 |
| --- | --- |
| `yasdd-orchestrator` | 开启新功能: 引导 → 架构(含自检 + 批次 + 测试) → 门控 → 按组件实现 → 测试 → 验证。 |
| `yasdd-quick-win` | 启动一次性的 quick win 流程: 引导 → 融合架构 → 实现 → 轻量评审。 |
| `yasdd-implement <slug>` | 从 STATE.md 恢复实现单个功能的组件。 |
| `yasdd-continue` | 恢复**所有**进行中功能,从其停下的确切步骤继续(引导、架构、门控、实现、测试、验证或收尾)。 |
...
| 技能 | 角色 |
| --- | --- |
| `yasdd-elicitation` | 分层批量引导(core 8 + extended 10 若复杂/greenfield); greenfield 检测 → 播种 CONVENTIONS.md; 每轮 Christel & Kang 监视清单; 对 REQUEST.md 的请求覆盖检查; 编写 ELICITATION.md。(主会话) |
| `yasdd-quick-elicitation` | quick win core-only 引导(8 节, 无 extended); greenfield 检测; 对 REQUEST.md 的请求覆盖检查; 编写 `.yasdd/quick-wins/<slug>/ELICITATION.md`。(主会话) |
...
**开发者应使用 `yasdd-spy`**(而非 harness 的通用 `explore` 代理)来进行技能要求的代码库调查。spy 从入口点到数据存储追踪功能实现,返回 `file:line` 引用和关键文件列表。它还检测 **greenfield** 仓库(无源文件)并返回 greenfield 信号,以便引导技能播种 `CONVENTIONS.md`。

要配置特定模型,请编辑 `agents/yasdd-spy.md` 并添加或更改 frontmatter 中的 `model:` 字段(支持取决于你的 agent harness)。

## 快速开始

1. 在项目中加载一次 `yasdd-init` skill(创建 `.yasdd/`、`config.yml`、`PROJECT-STATE.md` 并更新 `AGENTS.md`)。
2. 加载 `yasdd-orchestrator` skill 并回答关于你功能的批量提问。
...
`yasdd-init` skill 不创建 CONVENTIONS.md — 由引导/architect 在首个功能播种, 非 init 时(init 还不知道技术栈)。
...
.yasdd/
  ...
  features/<slug>/
    REQUEST.md                       # 用户的原始请求(raw $ARGUMENTS)
...
`yasdd-quick-win` skill 把完整 SDD 流水线折叠成单次、无状态的流程:
