...
| `yasdd-implementer` | 实现一个规范: 范围化读取、代码 + 最小测试、一致性表、增量写入 SUMMARY.md (Business/Implemented/Files), 返回 FINISHED/ISSUES。 |
| `yasdd-verifier` | 多轨道仅研究评审 + 一个**测试通过闸门**(在轨道前运行 lint/typecheck/tests)。 |
