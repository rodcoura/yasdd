...
- **一个 PLAN.md, 无规范拆分**: plan 技能对用户进行 grill(每次一个问题并附推荐答案), 通过 `yasdd-spy` 运行代码库探索(子代理或会话内), 编写唯一的 PLAN.md, 包含组件 `[M#]` + 内联并行标记 + Rules/Cases/Acceptance 带 anchors + Test impact。无单独的 elicitation/architecture 产物。
...
`yasdd-spy` 是唯一 `disable-model-invocation: false` 的技能 — 在 PLAN 阶段加载用于代码库调查。它从入口点到数据存储追踪功能实现, 返回 `file:line` 引用和关键文件列表。它还检测 **greenfield** 仓库(无源文件)并返回 greenfield 信号, 以便 plan 技能播种 `CONVENTIONS.md`。被要求时, 它将源文件映射到既有测试文件以填充 Test impact 节。

**如何运行**取决于 `explorerAgentName` 配置:

- **`explorerAgentName` 已设置**(如 `"general-purpose"`): plan 技能启动该命名的子代理, 传递功能请求 + 加载 `yasdd-spy` 技能的指令。当范围不确定时, 多个 spy 可并行运行(至 `maxParallelism`)— 每个分配不同的关注领域。
- **`explorerAgentName` 为空**(默认): plan 技能直接在主会话中加载 `yasdd-spy` 并内联探索(顺序, 无子代理)。复用已加载的代码上下文, 无子代理开销。

要在特定模型上运行 spy, 请编辑 `skills/yasdd-spy/SKILL.md` 并添加或更改 frontmatter 中的 `model:` 字段(支持取决于你的 agent harness)。
...
3. plan 技能对你进行 grill(每次一个问题并附推荐答案), 通过 `yasdd-spy` 运行代码库探索(若 `explorerAgentName` 已设置则用子代理, 为空则在会话内), 编写 `PLAN.md`。它验证计划并呈现给你接受。
...
explorerAgentName: ""   # 用于代码库探索的子代理名称(加载 yasdd-spy 技能); 空值 = 在主会话中探索
```
