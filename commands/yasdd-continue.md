...
For each chosen feature, run the **IMPLEMENT LOOP (step 5)** + **WRAP UP (step 6)** from the `/yasdd` playbook with the `autoMode` rules, in `STATE.md` spec order. Pass the spec path + config values (`autoMode`, `maxParallelism`, `maxSpecs`) to each implementer/verifier subagent and tell them to use those values instead of re-reading `config.yml`. Report each feature's outcome after it finishes. Honor the implementer FINISHED/ISSUES return protocol from step 5b.
