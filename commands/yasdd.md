...
You are the orchestrator. Read `.yasdd/config.yml` once at the start (create it with `autoMode: false`, `maxParallelism: 3` if missing — no `maxSpecs` in v2). `autoMode` drives the gate at step 3; `maxParallelism` caps parallel subagent calls per step (ELICITATION yasdd-spy, IMPLEMENT batch size, verifier tracks).
...
4. **IMPLEMENT LOOP** (parallel per batch, code-only, no checks):
   ...
   b. Read `ARCHITECTURE.md`'s `Parallel batches` section. For each batch: launch up to `maxParallelism` implementer subagents (skill `yasdd-implementer`) in PARALLEL, one per component `[M#]` in the batch. Each implementer gets the component ID `[M#]` + ARCHITECTURE.md path. Each returns `FINISHED` + one-line summary + conformance table (functioning DEFERRED) + changed-files manifest, or `ISSUES` + brief issues. The implementer already increments `SUMMARY.md` on either outcome. File ownership is disjoint within a batch (no write races).
      - `FINISHED` → mark component `[x]`-ish in `STATE.md` (set `impl: done`), bump count, update `PROJECT-STATE.md`. Collect the manifest + conformance table for the tester.
   ...
   d. **Aggregate** manifests + conformance tables across all batches. **Mid-flight batch update (in-memory):** if two components in the same batch report the same file in their changed-files manifests (file conflict the architect didn't foresee), demote the overlapping components to sequential (in-memory only — no ARCHITECTURE.md rewrite). Flag to the user: "M2 and M3 both modified `utils/auth.ts` — ran sequentially instead of parallel." The aggregated manifest + conformance tables become the tester's input.
