...
4. Record dense structured notes to `.yasdd/quick-wins/<slug>/DISCUSS.md` under headings: Goal & why, I/O, Constraints, Edge & error cases, Dependencies on existing code, Non-goals, Unknowns (bullets, not prose). Keep an explicit `## Open questions` list; close resolved items.
