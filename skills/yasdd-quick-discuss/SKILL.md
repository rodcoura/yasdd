...
## Process
1. Confirm the kebab-case slug for the quick win. Create `.yasdd/` (if missing), `.yasdd/quick-wins/<slug>/`, and `DISCUSS.md` if missing. Read `.yasdd/config.yml` for `maxParallelism`; if missing, create it with `autoMode: false`, `maxParallelism: 3`, `maxSpecs: 5`, and an empty `gate: { testCmd: "", lintCmd: "", typecheckCmd: "" }` block.
...
8. Write a final `## Summary` (handoff to spec) and proceed to write the quick-win spec (skill `yasdd-quick-spec`).
