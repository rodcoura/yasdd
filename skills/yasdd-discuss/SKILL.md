...
## Process
1. Confirm kebab-case slug. Create `.yasdd/`, `.yasdd/features/<slug>/`, and `DISCUSS.md` if missing. Create `.yasdd/PROJECT-STATE.md` (`# Project State` + empty `## Features`) if missing. Read `.yasdd/config.yml` for `maxParallelism`; if missing, create it with `autoMode: false`, `maxParallelism: 3`, `maxSpecs: 5`, and an empty `gate: { testCmd: "", lintCmd: "", typecheckCmd: "" }` block.
...
8. Write a final `## Summary` (handoff to design) and proceed to design (skill `yasdd-designer`).
