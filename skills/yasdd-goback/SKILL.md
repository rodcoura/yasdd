...
## Process
1. Read `.yasdd/features/<slug>/SUMMARY.md`, `ARCHITECTURE.md`, and `STATE.md` into context. If missing, STOP and tell the user. Read `.yasdd/config.yml` for `autoMode` and `maxParallelism` (defaults: `autoMode: false`, `maxParallelism: 3`); read-only — do NOT create it (only `yasdd-orchestrator`/`yasdd-init`/`yasdd-status`/`yasdd-quick-win` create it).
...
6. Tell the user to load the `yasdd-implement` skill (with `<slug>`) to implement the new component.
