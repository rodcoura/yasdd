...
4. **IMPLEMENT LOOP** (parallel per batch, code-only, no checks):
   ...
   b. Read `ARCHITECTURE.md`'s `Parallel batches` section. For each batch: launch up to `maxParallelism` implementer subagents (skill `yasdd-implementer`) in PARALLEL, one per component `[M#]` in the batch. Each implementer gets the component ID `[M#]` + ARCHITECTURE.md path. Each returns `FINISHED` + one-line summary + conformance table (functioning DEFERRED) + changed-files manifest, or `ISSUES` + brief issues. The implementer already updates `SUMMARY.md` on either outcome (bullets on `FINISHED`, blocked note on `ISSUES`). **SUMMARY.md is the implementer's responsibility; `STATE.md` is YOURS.** STATE.md is shared mutable state across the whole feature — implementers run in parallel with disjoint file sets, so they cannot safely write it; the orchestrator must. File ownership is disjoint within a batch (no write races).
   c. **Update `STATE.md`** (after each implementer returns, before the next batch): for the component that just returned, set its sub-marker in `.yasdd/features/<slug>/STATE.md`:
      - `FINISHED` → set `impl: done` (top-level marker stays `[ ]` until impl+test+verify are all done), bump count, update `PROJECT-STATE.md`. Collect the manifest + conformance table for the tester.
      - `ISSUES` → `autoMode: false`: surface the issues to the user and pause for direction. `autoMode: true`: set `impl: blocked` (top-level marker `[~]`), proceed.
   d. Next batch (sequential across batches, parallel within a batch).
   e. **Aggregate** manifests + conformance tables across all batches. **Mid-flight batch update (in-memory):** if two components in the same batch report the same file in their changed-files manifests (file conflict the architect didn't foresee), re-run the overlapping components sequentially (re-launch them one at a time, in `STATE.md` order, so each sees the previous one's output; in-memory only — no ARCHITECTURE.md rewrite). Flag to the user: "M2 and M3 both modified `utils/auth.ts` — re-ran sequentially to reconcile." The reconciled manifest + conformance tables become the tester's input.
   (Verification is deferred to step 5 — ONE feature-level pass.)
   **Checkpoint before TEST:** confirm every implemented component in `STATE.md` shows `impl: done` (or `impl: blocked`); if any is still `impl: pending`, update it now before launching the tester.
