...
## Process
1. Confirm the kebab-case slug for the quick win. Create `.yasdd/` (if missing), `.yasdd/quick-wins/<slug>/`, and `ELICITATION.md` if missing. Read `.yasdd/config.yml` for `autoMode` and `maxParallelism`; if missing, use defaults `autoMode: false`, `maxParallelism: 3` (do NOT create config.yml — only `yasdd-orchestrator`, `yasdd-init`, `yasdd-status`, and `yasdd-quick-win` create it). Read `.yasdd/quick-wins/<slug>/REQUEST.md` (the verbatim user request) as the authoritative source of constraints.
...
4. Record dense structured notes to `.yasdd/quick-wins/<slug>/ELICITATION.md` under the **core-only** headings (8 sections, no extended): Problem & motivation, Goal & success measures, Data shapes, Interface contracts, Happy-path flow, Invariants, Acceptance criteria, Non-goals. Keep an explicit `## Open questions` list; close resolved items. Also keep a `## Request constraints` checklist: enumerate every explicit constraint/requirement stated in `REQUEST.md` (one item each, quoted verbatim where possible) — this is the traceability list the stop-gate (step 6) checks against. The three headings must be concrete enough to quote into the architecture:
...
- Do NOT create `ARCHITECTURE.md` (the quick-architect writes the simplified ARCHITECTURE.md).
