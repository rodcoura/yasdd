...
You are the orchestrator. Read `.yasdd/config.yml` once at the start (create it with `autoMode: false`, `maxParallelism: 3` if missing). `autoMode` drives the gate at step 3; `maxParallelism` caps parallel subagent calls per step (ELICITATION's yasdd-spy, IMPLEMENT batch size, verifier tracks).
